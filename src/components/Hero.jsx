...
export default function Hero({
  ...
  return (
    <section className="relative overflow-hidden py-24 sm:py-32">
      ...
      <div className="absolute top-1/3 left-1/3 -z-10 h-[300px] w-[300px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-indigo-500/10 blur-[100px]"></div>

      <div className="mx-auto max-w-4xl px-6 text-center">
        {/* Sub-badge */}
        <div className="mx-auto mb-6 flex max-w-fit items-center space-x-2 rounded-full border border-violet-500/30 bg-violet-500/10 px-4 py-1.5 text-xs font-semibold text-violet-400">
          <span>Introducing OTZ Marketplace</span>
          <span className="h-1 w-1 rounded-full bg-violet-400"></span>
          <span className="text-slate-300">The Future of Media Booking</span>
        </div>

        {/* Hero Heading */}
        <h1 className="text-4xl font-extrabold tracking-tight text-white sm:text-6xl">
          Discover & Book <br />
          <span className="bg-gradient-to-r from-violet-400 via-indigo-300 to-purple-400 bg-clip-text text-transparent drop-shadow-[0_2px_10px_rgba(139,92,246,0.15)]">
            Advertising Spaces
          </span>
        </h1>
        ...
        <form
          ...
          className="mx-auto mt-10 max-w-2xl rounded-2xl border border-slate-800 bg-slate-900/60 p-2 shadow-2xl backdrop-blur-md sm:rounded-full"
        >
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
            ...
            <div className="flex flex-1 items-center px-3 py-2">
              ...
              <input
                ...
                className="ml-2.5 w-full bg-transparent text-sm text-slate-100 placeholder-slate-500 outline-none"
              />
            </div>

            {/* Separator Line (Desktop only) */}
            <div className="hidden h-8 w-px bg-slate-800 sm:block"></div>
